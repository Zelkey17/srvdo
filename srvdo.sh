...
: "${SRVDO_CHECK_SPACE:=1}"            # проверять место перед push и bak
: "${SRVDO_RECONNECT:=1}"
...
# ── совместимость с zsh ───────────────────────────────────────────────────
# read -p и read -s в zsh значат другое, поэтому ввод — через обёртки.
if [ -n "${ZSH_VERSION-}" ]; then
  _srvdo_read()  { printf '%s' "$2" >&2; IFS= read -r "$1"; }
  _srvdo_reads() { printf '%s' "$2" >&2; IFS= read -rs "$1"; printf '\n' >&2; }
else
  _srvdo_read()  { IFS= read -r -p "$2" "$1"; }
  _srvdo_reads() { IFS= read -r -s -p "$2" "$1"; printf '\n' >&2; }
fi

# mapfile есть только в bash
_srvdo_lines_to() {   # имя_массива < stdin
  local __n=$1 __l
  eval "$__n=()"
...
_srvdo_help() {
  printf '%ssrvdo%s — сервер под рукой: команды, сессии, файлы, порты.  сервер: %s%s%s\n' \
    "$_CB" "$_C0" "$_CB" "${_SRVDO_H:-$SRVDO_HOST}" "$_C0"
...
;;
    flush) cat <<EOF
Сбросить зависшие ssh-соединения.

  srvdo flush                      текущий сервер + мёртвые сокеты
  srvdo flush --all                закрыть соединения со всеми серверами

Зачем: мультиплексирование (ControlMaster) держит одно соединение и
переиспользует его — за счёт этого команды и Tab работают мгновенно. Но если
сеть оборвалась некрасиво, сокет остаётся, а мастер-процесс уже мёртв: тогда
любая команда молча ждёт таймаута вместо того, чтобы подключиться заново.

flush закрывает мастер-соединение (ssh -O exit), удаляет сокеты, которые не
отвечают на ssh -O check, и сбрасывает кэш подсказок.

Признаки, что пора: команды подвисают на десятки секунд, хотя srvdo ping
показывает, что порт открыт и ключ принимается.
EOF
;;
    ping) cat <<EOF
...
_srvdo_rsync() {
  local -a e; _srvdo_lines_to e < <(_srvdo_excl)
  [ "${_SRVDO_DRY:-0}" = 1 ] && e+=(--dry-run)
  rsync "${e[@]}" "$@"
}

_srvdo_resolve() {
  ssh -G "${1:-${_SRVDO_H:-$SRVDO_HOST}}" 2>/dev/null \
...
_srvdo_diag() {
  local host=${1:-${_SRVDO_H:-$SRVDO_HOST}}
  printf '\n' >&2
...
_srvdo_ssh() {
  ...
  [ "${1-}" = "--tty" ] && { tty=1; shift; }
  local rc
  if [ $tty -eq 1 ]; then ssh -t "${_SRVDO_H:-$SRVDO_HOST}" "$@"; rc=$?
  else ssh "${_SRVDO_H:-$SRVDO_HOST}" "$@"; rc=$?; fi
...
_srvdo_require_tmux() {
  ...
  if ssh -o ConnectTimeout="$SRVDO_CONNECT_TIMEOUT" "${_SRVDO_H:-$SRVDO_HOST}" 'command -v tmux >/dev/null' 2>/dev/null; then
...
_srvdo_have()  { ssh "${_SRVDO_H:-$SRVDO_HOST}" "tmux has-session -t=$(_srvdo_q "$1")" 2>/dev/null; }
_srvdo_names() { ssh -o BatchMode=yes "${1:-${_SRVDO_H:-$SRVDO_HOST}}" 'tmux ls -F "#{session_name}"' 2>/dev/null; }
...
_srvdo_exit_code() { ssh "${_SRVDO_H:-$SRVDO_HOST}" "cat $SRVDO_STATE/exit-$(_srvdo_q "$1") 2>/dev/null" 2>/dev/null; }
...
# ── свободное место ───────────────────────────────────────────────────────
_srvdo_hsize() {   # байты → человеко-читаемо
  awk -v b="${1:-0}" 'BEGIN{ n=split("Б,КБ,МБ,ГБ,ТБ", u, ","); i=1;
    while (b>=1024 && i<n) { b/=1024; i++ }
    if (i==1) printf "%d %s", b, u[i]; else printf "%.1f %s", b, u[i] }'
}

_srvdo_free_local()  { df -Pk "${1:-.}" 2>/dev/null | awk 'NR==2{print $4*1024; exit}'; }
_srvdo_free_remote() {
  ssh "${_SRVDO_H:-$SRVDO_HOST}" "df -Pk $(_srvdo_q "${1:-/}") 2>/dev/null || df -Pk /" 2>/dev/null \
    | awk 'NR==2{print $4*1024; exit}'
}
_srvdo_size_local() {
  local -a ex=(); local p
  for p in "${SRVDO_EXCLUDE[@]}"; do ex+=(--exclude="$p"); done
  du -sk "${ex[@]}" -- "$1" 2>/dev/null | awk '{print $1*1024; exit}'
}
_srvdo_size_remote() {
  ssh "${_SRVDO_H:-$SRVDO_HOST}" "du -sk $(_srvdo_q "$1") 2>/dev/null" 2>/dev/null | awk '{print $1*1024; exit}'
}

# нужно, свободно, куда, пояснение → 0 можно продолжать, 1 отказ
_srvdo_check_space() {
  local need=$1 free=$2 where=$3 note=$4
  case $need$free in *[!0-9]*|'') return 0 ;; esac   # не смогли измерить — не мешаем
  [ "$need" -gt 0 ] || return 0
  if [ "$free" -lt "$need" ]; then
    _srvdo_err "$where не хватит места: нужно ~$(_srvdo_hsize "$need"), свободно $(_srvdo_hsize "$free")"
    [ -n "$note" ] && _srvdo_hint "$note"
    _srvdo_confirm "всё равно продолжить?" || return 1
  elif [ "$free" -lt $((need * 2)) ]; then
    _srvdo_warn "$where останется мало места: нужно ~$(_srvdo_hsize "$need") из $(_srvdo_hsize "$free")"
  fi
  return 0
}

# ── ловушки прерывания ────────────────────────────────────────────────────
# Обработчик ставится только на время долгой операции и снимается после,
# чтобы не оставлять его в оболочке пользователя.
_srvdo_trap_on() {
  _SRVDO_TRAP_SAVED=
  [ -z "${ZSH_VERSION-}" ] && _SRVDO_TRAP_SAVED=$(trap -p INT TERM 2>/dev/null)
  trap "$1" INT TERM
}
_srvdo_trap_off() {
  trap - INT TERM
  [ -n "${_SRVDO_TRAP_SAVED-}" ] && eval "$_SRVDO_TRAP_SAVED"
  unset _SRVDO_TRAP_SAVED
}

_srvdo_confirm() {
  [ "${_SRVDO_YES:-0}" = 1 ] && return 0
  local yn; _srvdo_read yn "$1 (y/N): "
...
_srvdo_editor() {
  ...
  if command -v fzf >/dev/null 2>&1; then
    ...
    local n; _srvdo_read n "номер [1]: "; n=${n:-1}
...
_srvdo_ask() {
  local q=$1 def=$2 ans
  if [ -n "$def" ]; then _srvdo_read ans "$q [$def]: "; else _srvdo_read ans "$q: "; fi
  printf '%s' "${ans:-$def}"
}
...
EOF
  ...
  if ssh -o BatchMode=yes -o ConnectTimeout=5 "$alias" true 2>/dev/null; then
    ...
    if command -v sshpass >/dev/null; then
      local pw; _srvdo_reads pw "Пароль (Enter — ввести вручную далее): "
...
# ── ping ──────────────────────────────────────────────────────────────────
_srvdo_ping() {
  local host=${1:-${_SRVDO_H:-$SRVDO_HOST}} u h p tmp
...
srvdo() {
  ...
  local _SRVDO_H=$SRVDO_HOST SRVDO_DIR=$SRVDO_DIR SRVDO_PEEK=$SRVDO_PEEK
  ...
  while [ $# -gt 0 ]; do
    case $1 in
      --alias=*|--host=*|--server=*) _SRVDO_H=${1#*=} ;;
      --alias|--host|--server)       _SRVDO_H=${2-}; shift
                                     [ -n "$_SRVDO_H" ] || { _srvdo_err "--alias без значения"; return 2; } ;;
      --dir=*)      SRVDO_DIR=${1#*=} ;;
  ...
  case $cmd in
    ...
    flush)
      # Мультиплексирование ускоряет всё, но при обрыве сети мастер-соединение
      # может зависнуть, и тогда любая команда молча ждёт таймаута.
      local sock base target closed=0 killed=0
      if [ "${1-}" = --all ]; then
        for sock in "$HOME"/.ssh/cm-*; do
          [ -e "$sock" ] || continue
          base=${sock##*/cm-}; target=${base%:*}
          if ssh -o ControlPath="$sock" -O exit "$target" >/dev/null 2>&1; then
            closed=$((closed+1)); _srvdo_say "  закрыт: $base"
          else
            rm -f "$sock" && { killed=$((killed+1)); _srvdo_say "  удалён мёртвый сокет: $base"; }
          fi
        done
      else
        if ssh -O exit "$host" >/dev/null 2>&1; then
          closed=1; _srvdo_say "  закрыто мастер-соединение с $host"
        else
          _srvdo_say "  активного мастер-соединения с $host нет"
        fi
        # осиротевшие сокеты: имя вида cm-user@host:port позволяет их проверить
        for sock in "$HOME"/.ssh/cm-*; do
          [ -e "$sock" ] || continue
          base=${sock##*/cm-}; target=${base%:*}
          ssh -o ControlPath="$sock" -O check "$target" >/dev/null 2>&1 && continue
          rm -f "$sock" && { killed=$((killed+1)); _srvdo_say "  удалён мёртвый сокет: $base"; }
        done
      fi
      srvdo_cache_clear
      _srvdo_ok "закрыто: $closed, удалено мёртвых: $killed, кэш сброшен"
      _srvdo_cmd "srvdo ping   # проверить, что связь снова поднимается" ;;
    init)     _srvdo_init ;;
...
CFGEOF
    ...
    use)
      ...
      if [ "${2-}" = --save ]; then
        ...
        # тени больше нет: SRVDO_HOST не локальный, присваиваем напрямую
        SRVDO_HOST=$a
...
TMUXEOF
    ...
    push)
      ...
      if [ "$SRVDO_CHECK_SPACE" = 1 ] && [ "$_SRVDO_DRY" != 1 ]; then
        local need free
        need=$(_srvdo_size_local "$s"); free=$(_srvdo_free_remote "$d")
        _srvdo_check_space "$need" "$free" "на сервере в «$d»" \
          "это оценка по размеру источника; rsync пошлёт только изменившееся, так что реально может уйти меньше" \
          || return 1
      fi
    ...
    watch)
      ...
      local _stop=0 _runs=0
      _srvdo_trap_on '_stop=1'
      while [ "$_stop" -eq 0 ]; do
        inotifywait -rq -e modify,create,delete,move \
          --exclude '(\.git/|node_modules/|__pycache__/|\.swp$|~$)' "$s" >/dev/null 2>&1 || break
        [ "$_stop" -eq 0 ] || break
        sleep "$SRVDO_DEBOUNCE"
        if _srvdo_rsync -az --delete -- "$s" "$host:$d"; then
          _runs=$((_runs+1)); _srvdo_say "$(date +%H:%M:%S) ✓"
        else
          _srvdo_warn "$(date +%H:%M:%S) rsync вернул ошибку — продолжаю следить"
        fi
      done
      _srvdo_trap_off
      _srvdo_say "синхронизация остановлена (успешных прогонов: $_runs)" ;;
    ...
    bak)
      ...
      if [ "$SRVDO_CHECK_SPACE" = 1 ]; then
        local need free
        need=$(_srvdo_size_remote "$p"); free=$(_srvdo_free_local .)
        _srvdo_check_space "$need" "$free" "здесь, в $(pwd)," \
          "это размер каталога до сжатия — архив почти наверняка выйдет меньше" \
          || return 1
      fi
    ...
    tunnel)
      ...
      _srvdo_trap_on 'printf "\n"; _srvdo_say "туннель закрыт"'
      ssh -N -o ExitOnForwardFailure=yes -L "$lp:localhost:$rp" "$host"
      local rc=$?
      _srvdo_trap_off
      if [ $rc -ne 0 ] && [ $rc -ne 130 ] && [ $rc -ne 255 ]; then
        _srvdo_warn "ssh завершился с кодом $rc — возможно, порт $rp на сервере не слушается"
        _srvdo_cmd "srvdo ports"
      fi
      return 0 ;;
    ...
    mount)
      ...
      mkdir -p "$mp"
      # если прервать sshfs на полпути, каталог может остаться подмонтированным наполовину
      _srvdo_trap_on "printf '\n'; fusermount -u $(_srvdo_q "$mp") 2>/dev/null; \
                      _srvdo_warn 'монтирование прервано, точка $mp освобождена'"
      sshfs "$host:$SRVDO_DIR" "$mp" -o reconnect,ServerAliveInterval=15,ServerAliveCountMax=3,follow_symlinks
      local rc=$?
      _srvdo_trap_off
      if [ $rc -eq 0 ]; then _srvdo_ok "$host:$SRVDO_DIR → $mp"; _srvdo_cmd "srvdo umount"
      else _srvdo_err "смонтировать не удалось (код $rc)"; rmdir "$mp" 2>/dev/null; return $rc; fi ;;
...
_srvdo_names() {
  ssh -o BatchMode=yes -o ConnectTimeout=3 "${1:-${_SRVDO_H:-$SRVDO_HOST}}" \
      'tmux ls -F "#{session_name}" 2>/dev/null' 2>/dev/null \
    | grep -E '^[A-Za-z0-9_.:-]+$'
}

_srvdo_cached() {
  local cache=${TMPDIR:-/tmp}/.srvdo-${_SRVDO_H:-$SRVDO_HOST}-$UID out
...
_srvdo_remote_cmds() {
  local cache=${TMPDIR:-/tmp}/.srvdo-cmds-${_SRVDO_H:-$SRVDO_HOST}-$UID out
  if [ ! -f "$cache" ] || [ $(( $(date +%s) - $(stat -c %Y "$cache" 2>/dev/null || echo 0) )) -gt 3600 ]; then
    out=$(ssh -o BatchMode=yes -o ConnectTimeout=3 "${_SRVDO_H:-$SRVDO_HOST}" \
...
_srvdo_remote_paths() {
  printf '%s\n' "$1" | ssh -o BatchMode=yes -o ConnectTimeout=3 "${_SRVDO_H:-$SRVDO_HOST}" \
...
_srvdo_reply() {  # заполнить COMPREPLY вариантами, начинающимися с $1
  local cur=$1; shift
  local -a words=()
  _srvdo_lines_to words < <(printf '%s\n' "$@" | tr ' \t' '\n\n')
  COMPREPLY=()
  local w
  for w in "${words[@]}"; do
...
_srvdo_trim() {
  ...
  [ -n "$strip" ] || return 0
  local -a out=()
  for k in "${COMPREPLY[@]}"; do out+=("${k#"$strip"}"); done
  COMPREPLY=("${out[@]}")
}
...
_srvdo_complete() {
  local -a _W
  _srvdo_split
  # zsh нумерует массивы с 1, поэтому обходимся без числовых индексов
  local cur=${_W[-1]} prevw= total=0 w
  for w in "${_W[@]}"; do total=$((total+1)); done
  [ "$total" -ge 2 ] && prevw=${_W[-2]}

  local cmds="init ping doctor config hosts use examples help cache-clear flush
  ...
  local idx=0 skip=0 cmdidx=0 cmd=
  for w in "${_W[@]}"; do
    idx=$((idx+1))
    [ "$idx" -eq 1 ] && continue          # само «srvdo»
    [ "$idx" -eq "$total" ] && break      # последнее слово — то, что дополняем
    if [ "$skip" -gt 0 ]; then skip=$((skip-1)); continue; fi
    case $w in
      --alias|--host|--server|--dir|--editor|--peek|--timeout|--exclude|--config) skip=1 ;;
      -n|-y|-q|--*) ;;
      *) cmdidx=$idx; cmd=$w; break ;;
    esac
  done
  ...
  local pos=$((total - cmdidx))        # 1 — первый аргумент команды
  ...
  case $canon in
    ...
    flush)
      [ "$pos" -eq 1 ] && _srvdo_reply "$cur" "--all" || COMPREPLY=() ;;
    init|doctor|hosts|examples|keys|info|ports|list|tunnel|cache-clear|cache_clear)
      COMPREPLY=() ;;                    # аргументов не принимают
    *)
      # своя команда, для которой дополнять нечего — молчим;
      # всё остальное считаем командой для сервера и дополняем его путями
      local known
      known=" $(printf '%s' "$cmds" | tr -s '[:space:]' ' ') "
      case $known in
        *" $canon "*) COMPREPLY=() ;;
        *) _srvdo_reply_paths "$cur" < <(_srvdo_remote_paths "$cur"); _srvdo_nospace_if_dir ;;
      esac ;;
...
if [ -n "${ZSH_VERSION-}" ]; then
  # zsh умеет исполнять bash-функции дополнения через bashcompinit
  if ! command -v compdef >/dev/null 2>&1; then
    autoload -U +X compinit 2>/dev/null && compinit -C 2>/dev/null
  fi
  autoload -U +X bashcompinit 2>/dev/null && bashcompinit 2>/dev/null
  complete -F _srvdo_complete srvdo 2>/dev/null \
    || printf 'srvdo: дополнение в zsh недоступно (нужен bashcompinit)\n' >&2
else
